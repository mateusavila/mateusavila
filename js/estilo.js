function gerar_janela(url){
	$.get(url, function(data){$('.load_content2').html(data);});
	...
	$('#dialog').fadeIn(500);
	$('.window .close').css({opacity:'0.5'},200).hover(function(){$(this).animate({opacity: '1.0'},200);},function(){$(this).animate({opacity: '0.5'},200);});
	$('.window .close').click(function(e){e.preventDefault();$('#mask, .window').hide();});
	$('#mask').click(function(){$(this).hide();$('.window').hide();$('.window .load_content2').html("");});
}
function fixedNav(){$('header').addClass('fixedHeader');}
function staticNav(){$('header').removeClass('fixedHeader');}
function calculatePosition(){var position;var arrayPosition = [];var j = 0;$('.content').each(function(i){position = $(this).position();arrayPosition.push(position.top);j = i+1;$('#openPanel'+j).attr('positiontop',arrayPosition[i]);$('#panel'+j).attr('positiontop',arrayPosition[i]);});}
...
function calculateSlider(){$('#sliderContent #sliderContentList').css('width',$('body').width() * $('#sliderContent #sliderContentList section').length);}
...
$(document).ready(function(){
	...
	$(document).on('click', '#hideBanner', function(e){
		e.preventDefault();
		$('#banner').animate({opacity:0,height:0,'margin-top':'20px'},500);
		$(this).attr('id','showBanner').removeClass('hideBanner').addClass('showBanner').find('b').text('Mostrar Banner').parent().find('img').attr('src','images/downIcon.png');
	});
	$(document).on('click','#showBanner', function(e){
		e.preventDefault();
		$('#banner').animate({opacity:1,height:'180px','margin-top':'80px'},500);
		$(this).attr('id','hideBanner').removeClass('showBanner').addClass('hideBanner').find('b').text('Esconder Banner').parent().find('img').attr('src','images/upIcon.png');
	})
	...
	$(document).on('click','nav ul li a', function(e){
		e.preventDefault();
		id = $(this).attr('href').split('#link=')[1];
		$('#sliderContent #sliderContentList').animate({'left':-($('#sliderContent #sliderContentList #'+id).position().left)},500);
	});
	...
	$(function() {
		var step = 1; 
		var current = 0; 
		var maximum = $('#sliderContentList').find('section').size();
		var visible = 4; 
		var speed = 200; 
		var liSize = 960;
		...
		$(document).on('click', 'a#nextProf', function(){
			if(current + step < 0 || current + step > maximum - visible) {
				return false; 
			}else {
				current = current + step;
				$('#sliderContent section').animate({left: -(liSize * current)}, speed, null);
			}
			return false;
		});
 
		$(document).on('click', 'a#prevProf', function(){
			if(current - step < 0 || current - step > maximum - visible) {
				return false; 
			}else {
				current = current - step;
				$('#sliderContent #sliderContentList').animate({left: -(liSize * current)}, speed, null);
			}
			return false;
		});
	});
